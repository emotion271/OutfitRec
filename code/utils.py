...
class TrainDataset(Dataset):
    def __init__(self, visual_feature, text_feature, outfit_item, outfit_len):
        train = np.loadtxt("../dmr/new_data_train.txt", delimiter=',', dtype=int)
    ...
    def __getitem__(self, index):
        ...
        his_outfit_visual = cat([cat([torch.Tensor(visual_features[items]).unsqueeze(0) if items in visual_features.keys() else torch.zeros((1, 2048)).unsqueeze(0) for items in outfit_item[outfit.item()]], 1) for outfit in his_outfit], 0)
        outfit_text = cat([cat([torch.Tensor(text_features[items]).unsqueeze(0).unsqueeze(0) for items in outfit_item[outf.item()]], 1) for outf in outfit], 0)
        outfit_visual = cat([cat([torch.Tensor(visual_features[items]).unsqueeze(0) if items in visual_features.keys() else torch.zeros((1, 2048)).unsqueeze(0) for items in outfit_item[outf.item()]], 1) for outf in outfit], 0)
        ...
        outfit_mask = cat([torch.Tensor([1] * outfit_len[outf.item()] + [0] * (20 - outfit_len[outf.item()])).unsqueeze(0) for outf in outfit], 0)
...
class TestDataset(Dataset):
    def __init__(self, visual_feature, text_feature, outfit_item, outfit_len):
        train = np.loadtxt("../dmr/new_data_test.txt", delimiter=',', dtype=int)
        self.x_data = torch.from_numpy(train[:, 0:-1])
        self.y_data = torch.from_numpy(train[:, [-1]])
        self.visual_features = visual_feature
        self.text_features = text_feature
    ...
    def __getitem__(self, item):
        ...
        outfit_text = cat([cat(
            [torch.Tensor(text_features[items]).unsqueeze(0).unsqueeze(0) for items in outfit_item[outf.item()]], 1) for
                           outf in outfit], 0)
        outfit_visual = cat(
            [cat([torch.Tensor(visual_features[items]).unsqueeze(0) for items in outfit_item[outf.item()]], 1) for outf
             in outfit], 0)
        ...
        outfit_his_mask = cat(
            [torch.Tensor([1] * outfit_len[outfit.item()] + [0] * (20 - outfit_len[outfit.item()])).unsqueeze(0) for
             outfit in his_outfit], 0)
        outfit_mask = cat(
            [torch.Tensor([1] * outfit_len[outf.item()] + [0] * (20 - outfit_len[outf.item()])).unsqueeze(0) for outf in
             outfit], 0)
