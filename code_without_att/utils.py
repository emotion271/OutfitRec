...
class TrainDataset(Dataset):
    def __init__(self, path, visual_feature, text_feature, outfit_item, outfit_len):
        train = np.loadtxt(path, delimiter=',', dtype=int)
        self.x_data = torch.from_numpy(train[:, 0:-1])
        self.y_data = torch.from_numpy(train[:, [-1]])
        #self.x_data = list(train[:, 0:-1])
        #self.y_data = list(train[:, [-1]])
    ...
    def __getitem__(self, index):
        ...
        for outfit in his_outfit:
            for items in outfit_item[outfit.item()]:
                time3 = time.time()
                a = torch.Tensor(visual_features[items])
                time4 = time.time()
        '''
        #time5 = time.time()
        ...
        #his_outfit_visual = cat([cat([torch.Tensor(visual_features[items]) for items in outfit_item[outfit.item()]], 0).unsqueeze(0) for outfit in his_outfit], 0)
...
class TestDataset(Dataset):
    def __init__(self, visual_feature, text_feature, outfit_item, outfit_len):
        train = np.loadtxt("./test.txt", delimiter=',', dtype=int)
        self.x_data = torch.from_numpy(train[:, 0:-1])
        self.y_data = torch.from_numpy(train[:, [-1]])
        self.visual_features = visual_feature
        self.text_features = text_feature
