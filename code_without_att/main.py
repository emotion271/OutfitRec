...
my_config = {
    ...
    "model_file": r"./model_save/dmr111.model",
...
def trainning(model, train_data_loader, device, opt):
    ...
    for iteration, data in enumerate(train_data_loader):
        ...
        his_outfit_text, his_outfit_visual, mask, outfit_text, outfit_visual, outfit_his_mask, outfit_mask = his_outfit_text.to(device), his_outfit_visual.to(device), mask.to(device), outfit_text.to(device), outfit_visual.to(device), outfit_his_mask.to(device), outfit_mask.to(device)
        output = model(his_outfit_text, his_outfit_visual, mask, outfit_text, outfit_visual, outfit_his_mask, outfit_mask)
        #print(output)
...
def evaluating(model, test_loader, device):
    ...
    for iteration, data in enumerate(test_loader):
        ...
        targets = targets.to(device)
        his_outfit_text, his_outfit_visual, mask, outfit_text, outfit_visual, outfit_his_mask, outfit_mask = his_outfit_text.to(
            device), his_outfit_visual.to(device), mask.to(device), outfit_text.to(device), outfit_visual.to(
            device), outfit_his_mask.to(device), outfit_mask.to(device)
...
def F(batch_size, eb_size, device):
    ...
    visual_features['0'] = torch.Tensor(np.zeros((1,2048)))
    print('load v finish')
    #assert(0>1)
    ...
    trainDataset = TrainDataset('../data/new_data_train1.txt', visual_features, text_features, outfit_item, outfit_len)
    train_loader = DataLoader(trainDataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=4)

    testDataset = TrainDataset('../data/new_data_test1.txt', visual_features, text_features, outfit_item, outfit_len)
