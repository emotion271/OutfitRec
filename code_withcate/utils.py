...
class TrainDataset(Dataset):
    def __init__(self, path, visual_feature, text_feature, outfit_item, outfit_len, item_cate):
        train = np.loadtxt(path, delimiter=',', dtype=int)
    ...
    def __getitem__(self, index):
        ...
        his_outfit_cate = cat([cat([torch.IntTensor([item_cate[items]]).unsqueeze(0).unsqueeze(0) for items in outfit_item[outfit.item()]], 1) for outfit in his_outfit], 0)
        #time7 = time.time()
        ...
        outfit_cate = cat([torch.IntTensor([item_cate[items]]).unsqueeze(0).unsqueeze(0) for items in outfit_item[outfit.item()]], 1)
        ...
        '''
        item_id_list = [items for outfit in his_outfit for items in outfit_item[outfit.item()]]
        his_outfit_text = np.concatenate(([text_features[item] for item in item_id_list]), axis=0).reshape(-1, 20, 300)
        his_outfit_text = torch.from_numpy(his_outfit_text).to(torch.float32)

        his_outfit_visual = np.concatenate(([visual_features[item] for item in item_id_list]), axis=0).reshape(-1, 20, 2048)
        his_outfit_visual = torch.from_numpy(his_outfit_visual).to(torch.float32)

        his_outfit_cate = np.array([item_cate[item] for item in item_id_list]).reshape(-1, 20, 1)
        his_outfit_cate = torch.from_numpy(his_outfit_cate).to(torch.int)

        item_id_list = [items for items in outfit_item[outfit.item()]]
        outfit_text = np.concatenate(([text_features[item] for item in item_id_list]), axis=0).reshape(-1, 20, 300)
        outfit_text = torch.from_numpy(outfit_text).to(torch.float32)

        outfit_visual = np.concatenate(([visual_features[item] for item in item_id_list]), axis=0).reshape(-1, 20, 2048)
        outfit_visual = torch.from_numpy(outfit_visual).to(torch.float32)

        outfit_cate = np.array([item_cate[item] for item in item_id_list]).reshape(-1, 20, 1)
        outfit_cate = torch.from_numpy(outfit_cate).to(torch.int)

        outfit_his_mask = cat([torch.Tensor([1] * outfit_len[outfit.item()] + [0] * (20 - outfit_len[outfit.item()])).unsqueeze(0) for outfit in his_outfit], 0)
        outfit_mask = torch.Tensor([1] * outfit_len[outfit.item()] + [0] * (20 - outfit_len[outfit.item()])).unsqueeze(0)

        return his_outfit_text, his_outfit_visual, mask, outfit_text, outfit_visual, uid, outfit_his_mask, outfit_mask, his_outfit_cate, outfit_cate, target
...
class TestDataset(Dataset):
    def __init__(self, visual_feature, text_feature, outfit_item, outfit_len, item_cate):
        train = np.loadtxt("../data/new_data_test1.txt", delimiter=',', dtype=int)
        self.x_data = torch.from_numpy(train[:, 0:-1])
        self.y_data = torch.from_numpy(train[:, [-1]])
        self.visual_features = visual_feature
        self.text_features = text_feature
    ...
    def __getitem__(self, index):
        ...
        his_outfit_cate = cat(
            [cat([torch.Tensor(item_cate[items]).unsqueeze(0).unsqueeze(0) for items in outfit_item[outfit.item()]], 1)
             for outfit in his_outfit], 0)
        outfit_text = cat(
            [torch.Tensor(text_features[items]).unsqueeze(0).unsqueeze(0) for items in outfit_item[outfit.item()]], 1)
        outfit_visual = cat([torch.Tensor(visual_features[items]).unsqueeze(0) for items in outfit_item[outfit.item()]],1)
